...
my$lkc=0.3;
my$hkc=5;
#my@flyep_array=" --keep-haplotype -m 1000 ";
my$minoverlap=800;
#v1.0: March 07, 2024
#v1.1: May 09, 2024; 
...
if ($help or not defined $fastq ) {
	...
	#print "  -y: parameters for flye (-y --meta -y --keep-haplotype) (default: --keep-haplotype).\n";
...
if( $organelle eq 'organelle' ){
	...
	#		if( $trf_out[1]-$trf_out[0] > $hash_mito_len{$chr}*0.8 && $trf_out[2] > 50 && $trf_out[2] < 300 ){
